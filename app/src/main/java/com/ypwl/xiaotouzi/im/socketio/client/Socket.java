...
import com.ypwl.xiaotouzi.im.socketio.hasbinary.HasBinary;
import com.ypwl.xiaotouzi.im.socketio.parser.Packet;
...
public class Socket extends Emitter {
    ...
    private static final String TAG = "ymseeage:" + Socket.class.getName();
    ...
    public Socket setNsp(String nsp) {
        this.nsp = nsp;
        return this;
    }
    ...
    public Emitter emit(final String event, final Object... args) {
        EventThread.exec(new Runnable() {
            ...
            public void run() {
                ...
                LogUtil.e(TAG, "Data Packet hasBinary? = " + (parserType == Parser.BINARY_EVENT));
                Packet<JSONArray> packet = new Packet<>(parserType, jsonArgs);

                if (_args.get(_args.size() - 1) instanceof Ack) {
                    LogUtil.e(TAG, String.format("emitting packet with ack id %d", Socket.this.ids));
    ...
    public Emitter emit(final String event, final Object[] args, final Ack ack) {
        EventThread.exec(new Runnable() {
            ...
            public void run() {
                ...
                LogUtil.e(TAG, String.format("emitting packet with ack id %d", ids));
    ...
    private void onopen() {
        LogUtil.e(TAG, "transport is open - connecting");
    ...
    private void onclose(String reason) {
        LogUtil.e(TAG, String.format("close (%s)", reason));
    ...
    private void onevent(Packet<JSONArray> packet) {
        ...
        LogUtil.e(TAG, String.format("emitting event %s", args));

        if (packet.id >= 0) {
            LogUtil.e(TAG, "attaching ack callback to event");
            ...
            public void call(final Object... args) {
                EventThread.exec(new Runnable() {
                    ...
                    public void run() {
                        if (sent[0]) return;
                        sent[0] = true;
                        LogUtil.e(TAG, String.format("sending ack %s", args.length != 0 ? args : null));
    ...
    private void onack(Packet<JSONArray> packet) {
        ...
        if (fn != null) {
            LogUtil.e(TAG, String.format("calling ack %s with %s", packet.id, packet.data));
            fn.call(toArray(packet.data));
        } else {
            LogUtil.e(TAG, String.format("bad ack %s", packet.id));
        }
    }
    ...
    private void ondisconnect() {
        LogUtil.e(TAG, String.format("server disconnect (%s)", this.nsp));
        this.destroy();
    ...
    public Socket close() {
        EventThread.exec(new Runnable() {
            ...
            public void run() {
                if (Socket.this.connected) {
                    LogUtil.e(TAG, String.format("performing disconnect (%s)", Socket.this.nsp));
    ...
    private static Object[] toArray(JSONArray array) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            try {
                ...
                LogUtil.e(TAG, "An error occured while retrieving data from JSONArray", e);
                v = null;
            }
