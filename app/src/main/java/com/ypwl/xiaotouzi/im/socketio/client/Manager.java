...
import com.ypwl.xiaotouzi.im.socketio.backo.Backoff;
...
public class Manager extends Emitter {
    ...
    private static final String TAG ="ymseeage:" +  Manager.class.getName();
    ...
    public Manager open(final OpenCallback fn) {
        EventThread.exec(new Runnable() {
            @Override
            public void run() {
                LogUtil.e(TAG, String.format("readyState %s", Manager.this.readyState));
                if (Manager.this.readyState == ReadyState.OPEN || Manager.this.readyState == ReadyState.OPENING)
                    return;

                LogUtil.e(TAG, String.format("opening %s", Manager.this.uri));
                ...
                if (Manager.this._timeout >= 0) {
                    ...
                    LogUtil.e(TAG, String.format("connection attempt will timeout after %d", timeout));
                    ...
                    timer.schedule(new TimerTask() {
                        ...
                        public void run() {
                            EventThread.exec(new Runnable() {
                                @Override
                                public void run() {
                                    LogUtil.e(TAG, String.format("connect attempt timed out after %d", timeout));
                                    openSub.destroy();
                                    socket.close();
    ...
    /*package*/ void packet(Packet packet) {
        LogUtil.e(TAG, String.format("writing packet %s", packet));
        final Manager self = this;
    ...
    private void reconnect() {
        ...
        if (this.backoff.getAttempts() >= this._reconnectionAttempts) {
            LogUtil.e(TAG, "reconnect failed");
            this.backoff.reset();
            this.emitAll(EVENT_RECONNECT_FAILED);
            this.reconnecting = false;
        } else {
            long delay = this.backoff.duration();
            LogUtil.e(TAG, String.format("will wait %dms before reconnect attempt", delay));
                ...
                public void run() {
                    EventThread.exec(new Runnable() {
                        ...
                        public void run() {
                            ...
                            LogUtil.e(TAG, "attempting reconnect");
                            ...
                            self.open(new OpenCallback() {
                                ...
                                public void call(Exception err) {
                                    if (err != null) {
                                        LogUtil.e(TAG, "reconnect attempt error");
                                        self.reconnecting = false;
                                        self.reconnect();
                                        self.emitAll(EVENT_RECONNECT_ERROR, err);
                                    } else {
                                        LogUtil.e(TAG, "reconnect success");
                                        self.onreconnect();
                                    }
                                }
                            });
