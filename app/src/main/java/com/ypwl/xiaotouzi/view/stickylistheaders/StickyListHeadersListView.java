package com.ypwl.xiaotouzi.view.stickylistheaders;
...
import com.ypwl.xiaotouzi.view.stickylistheaders.WrapperViewList.LifeCycleListener;
...
public class StickyListHeadersListView extends FrameLayout {
    ...
    /** 悬浮Header显示状态的监听 */
    public interface OnHeaderShowStateListener {
        void OnHeaderShowState(boolean mIsShow);
    }
    /**
    ...
    /* --- Other --- */
    private OnHeaderShowStateListener mOnHeaderShowStateListener;
    ...
    private void updateOrClearHeader(int firstVisiblePosition) {
        ...
        if (!doesListHaveChildren || isHeaderPositionOutsideAdapterRange || isFirstViewBelowTop) {
            /** 悬浮header未显示 */
            setOnHeaderShowState(false);
            clearHeader();
            return;
        }
        /** 悬浮header显示 */
        setOnHeaderShowState(true);
        updateHeader(headerPosition);
    }

    /** 更新悬浮header 的显示状态 */
    private void setOnHeaderShowState(boolean mState){
        if (null != mOnHeaderShowStateListener){
            mOnHeaderShowStateListener.OnHeaderShowState(mState);
        }
    }
    ...
    /** 设置悬浮header显示状态监听 */
    public void setOnHeaderShowStateListener(OnHeaderShowStateListener listener){
        mOnHeaderShowStateListener = listener;
    }
