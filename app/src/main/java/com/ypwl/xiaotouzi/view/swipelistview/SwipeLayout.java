...
public class SwipeLayout extends FrameLayout implements SwipeLayoutInterface {
    ...
    public SwipeLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        // 初始化手势识别器
        mGestureDetector = new GestureDetectorCompat(context, new SimpleOnGestureListener() {
            @Override
    ...
    ViewDragHelper.Callback mCallback = new ViewDragHelper.Callback() {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            // 根据返回的boolean值决定是否要滑动刚刚按下的child。
    ...
    private Rect computeBackLayoutViaFront(Rect frontRect) {
        int bl = frontRect.left, bt = frontRect.top, br = frontRect.right, bb = frontRect.bottom;
        if (mShowEdge == ShowEdge.Left) {
            bl = frontRect.left - mDragDistance;
