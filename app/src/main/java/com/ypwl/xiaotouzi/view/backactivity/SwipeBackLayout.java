package com.ypwl.xiaotouzi.view.backactivity;
...
public class SwipeBackLayout extends FrameLayout {
    ...
    private static final int DEFAULT_EDGE_SIZE = 15;// dip
    private static final ColorDrawable DEFAULT_SHADOW_DRAWABLE = new ColorDrawable(Color.TRANSPARENT);
    ...
    /**
     * Threshold of scroll, we will close the activity, when scrollPercent over  this value, with default value;
     */
    ...
    private void initDefault() {
        ...
        setEdgeSize(BAUtil.dip2px(getContext(), DEFAULT_EDGE_SIZE));//默认边缘滑动触摸范围宽度
        setEdgeTrackingEnabled(EDGE_FLAGS[0]);//默认左侧可滑
        setShadow(DEFAULT_SHADOW_DRAWABLE, EDGE_LEFT);//默认左侧背景阴影
        setShadow(DEFAULT_SHADOW_DRAWABLE, EDGE_RIGHT);//默认右侧背景阴影
        setShadow(DEFAULT_SHADOW_DRAWABLE, EDGE_BOTTOM);//默认底部背景阴影
        //设置默认滑动速率
        final float minVel = DEFAULT_MIN_FLING_VELOCITY * getResources().getDisplayMetrics().density;
     ...
     * callback's methods will only be invoked for edges for which edge tracking has been enabled.
     *
