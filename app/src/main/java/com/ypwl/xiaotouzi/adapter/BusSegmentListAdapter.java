...
import com.ypwl.xiaotouzi.bean.SchemeBusStepBean;
...
public class BusSegmentListAdapter extends BaseAdapter {
	...
	private List<SchemeBusStepBean> mBusStepList = new ArrayList<SchemeBusStepBean>();

	public BusSegmentListAdapter(Context context, List<BusStep> list) {
		this.mContext = context;
		SchemeBusStepBean start = new SchemeBusStepBean(null);
		start.setStart(true);
		mBusStepList.add(start);
		for (BusStep busStep : list) {
			if (busStep.getWalk() != null) {
				SchemeBusStepBean walk = new SchemeBusStepBean(busStep);
				walk.setWalk(true);
				mBusStepList.add(walk);
			}
			if (busStep.getBusLine() != null) {
				SchemeBusStepBean bus = new SchemeBusStepBean(busStep);
				bus.setBus(true);
				mBusStepList.add(bus);
			}
		}
		SchemeBusStepBean end = new SchemeBusStepBean(null);
		end.setEnd(true);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		final SchemeBusStepBean item = mBusStepList.get(position);
		if (position == 0) {
	...
	private class ArrowClick implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (mHolder.arrowExpend == false) {
				...
				mHolder.busExpandImage.setImageResource(R.mipmap.btn_018_reverse_select);
				...
				mHolder.busExpandImage.setImageResource(R.mipmap.btn_018_select);
				mHolder.expandContent.removeAllViews();
			}
		}

		private void addBusStation(BusStationItem station) {
			LinearLayout ll = (LinearLayout) View.inflate(mContext, R.layout.item_bus_segment_ex, null);
