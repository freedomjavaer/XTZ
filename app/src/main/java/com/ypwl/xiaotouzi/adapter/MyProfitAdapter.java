...
public class MyProfitAdapter extends BaseAdapter {
    ...
    public MyProfitAdapter(ProfitRangeBean bean, int type, Context context) {
        this.bean = bean;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            if (type == 1){
                convertView = View.inflate(context, R.layout.item_profit_range_one, null);
            }else {
                convertView = View.inflate(context, R.layout.item_profit_range_two, null);
            }
        ...
        if (type == 1) {//平台
            ...
            float percent = (float) (Double.parseDouble(bean.getList().get(position).getProfit().replaceAll(",", ""))/Double.parseDouble(bean.getList().get(0).getProfit().replaceAll(",", "")));
            if (percent < 0.06f){
                percent = 0.06f;
            }
            ...
            viewHolder.mNumberOne.setText(bean.getList().get(position).getProfit() + "");
            viewHolder.mNumberTwo.setText("年化率" + bean.getList().get(position).getRate() + "%");
        } else {//标
            viewHolder.mSerialNumber = (TextView) convertView.findViewById(R.id.tv_serial_number);
            viewHolder.mRLContent = (RelativeLayout) convertView.findViewById(R.id.rl_profit_item_content);
            viewHolder.mPlatformName = (TextView) convertView.findViewById(R.id.tv_profit_range_platform_name);
